...
USE_SCIKITS_UMFPACK = (
    False  ## This uses UMFPACK backend for scipy instead of naive scipy.
)
...
class SparseMat:
    ...
    def to_coo(self):
        return coo_matrix(
            (self.vals, (self.inds[0, :], self.inds[1, :])), shape=(self.n, self.m)
        )
    ...
    def to_cholesky(self):
        return CholeskySolverD(
            self.n, self.inds[0, :], self.inds[1, :], self.vals, MatrixType.COO
        )
        ...
        if USE_TORCH_SPARSE:
            ...
            if self.vals.device.type == "cpu":
                tensor_zero_hack = torch.FloatTensor(
                    [0]
                ).double()  # This line was somehow responsible for a nasty NAN bug
            else:
                tensor_zero_hack = (
                    torch.cuda.FloatTensor([0]).to(dense.get_device()).double()
                )
            # beware with addmm, it is experimental and gave me a NaN bug!
            res = torch.sparse.addmm(
                tensor_zero_hack,
                torch.sparse_coo_tensor(self.inds.double(), self.vals.double()),
                (dense.double()),
            ).type_as(self.vals)
...
class PoissonSystemMatrices:
    ...
    def __init__(
        self, V, F, grad, rhs, w, ttype, is_sparse=True, lap=None, cpuonly=False
    ):
        self.dim = 3
    ...
    def compute_poisson_solver_from_laplacian(self, compute_splu=True):
        ...
        if compute_splu:
            self.compute_splu()
        return self.create_poisson_solver_from_splu(
            self.__splu_L, self.__splu_U, self.__splu_perm_c, self.__splu_perm_r
        )
    ...
    def compute_laplacian(self):
        ...
        if (
            isinstance(self.lap, PoissonSystemMatrices)
            and self.lap.vals.shape[0] == self.__V.shape[0]
        ):
            assert False, (
                "this should not happen, the fix is to remove a column and row of the laplacian"
            )
            self.lap = self.lap[1:, 1:]
    ...
    def get_new_grad(self):
        ...
        self.igl_grad = SparseMat.from_M(
            _convert_sparse_igl_grad_to_our_convention(grad.tocsc()), torch.float64
        )
...
def _convert_sparse_igl_grad_to_our_convention(input):
    ...
    r_c_data = np.hstack(
        (T.row[..., np.newaxis], T.col[..., np.newaxis], T.data[..., np.newaxis])
    )  # horizontally stack row, col and data arrays
...
class PoissonSolver:
    ...
    def solve_poisson(self, jacobians):
        ...
        c = torch.mean(sol, axis=1).unsqueeze(
            1
        )  ## Beware the predicted mesh is centered here.
...
def poisson_system_matrices_from_mesh(
    V, F, dim=3, ttype=torch.float64, is_sparse=True, cpuonly=False
):
    """
    ...
    grad = SparseMat.from_M(
        _convert_sparse_igl_grad_to_our_convention(grad), torch.float64
    )
    ...
    def solve(solver, b):
        ...
        if USE_CUPY:
            ...
            with cupy.cuda.Device(solver.device()):
                ...
                for i in range(
                    b_cupy.shape[2]
                ):  # b may have multiple columns, solve for each one
...
def _predicted_jacobians_to_vertices_via_poisson_solve(Lap, rhs, jacobians):
    ...
    def _list_rearrange_input(input):
        assert isinstance(input, list) and all(
            [isinstance(x, torch.Tensor) and len(x.shape) in [2, 3] for x in input]
        )
        P = []
        for p in input:
...
class MyCuSPLU:
    ...
    def solve(self, b):
        ...
        assert b.device.id == self.__device, (
            "got device" + str(b.device.id) + "instead of" + str(self.__device)
        )
        # st = time.time()
        try:  # unit_diagonal is a new kw
            c = spsolve_triangular(
                self.L, b, lower=True, unit_diagonal=True, overwrite_b=True
            )
        except TypeError:
